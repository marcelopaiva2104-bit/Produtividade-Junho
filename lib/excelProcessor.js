...
function personKey(value) {
  ...
  if (key === "ANA LUIZA ENTREPONTES DA COSTA") return "ANA LUIZA ENTREPORTES DA COSTA";
...
function mergeCounts(people, repasseCounts, reservaCounts, cancelCounts, distratoCounts, previousRepasseCounts = null) {
  const byKey = new Map(people.map((p) => [p.key, p]));
...
function buildDashboard(people, managerMap, files, period, repasseTeamCounts = new Map(), realtyReservations = { totals: new Map(), details: [] }, repasseAreaCounts = new Map(), structure = { teams: new Map() }, entryDates = new Map(), yearRepasses = null) {
  ...
  for (const person of people) {
    ...
    if (teamOverride) {
      ...
      person.area = areaOf(teamOverride);
    }
...
async function processExcelFiles(paths, savedFiles = []) {
  ...
  const people = mergeCounts(structured.people, repasses, reservas, cancelados, distratos, previousRepasses);
  return buildDashboard(people, managerMap, savedFiles, period, repasses.teamCounts, realtyReservations, repasses.areaCounts, structure, entryDates, yearRepasses);
}
