...
function buildDashboard(people, managerMap, files, period, repasseTeamCounts = new Map(), realtyReservations = { totals: new Map(), details: [] }, repasseAreaCounts = new Map(), structure = { teams: new Map() }, entryDates = new Map(), yearRepasses = null) {
  ...
  for (const person of people) {
    ...
    person.profile = repasseProfile(person.repassesMonth);
  }
  ...
  const teams = [...teamMap.values()].map((team) => {
    ...
    const profiles = profileBreakdown(productionActive);
    return { ...team, above, below, critical, teamGoal, repasses, reservas, pastas, cancelados, distratos, ipcYear, ipcMonth, activePeople: productionActive.length, profiles };
  });
