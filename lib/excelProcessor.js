...
const TEAM_MANAGER_OVERRIDES = new Map([
  ...
  ["IMOBILIARIAS 2 | AGL", "Imobiliarias"],
  ["IMOBILIARIAS 2 | CAT", "Imobiliarias"],
  ["IMOBILIARIAS 2 | FSA", "Imobiliarias"],
  ["EQUIPE PROPRIA 2 | CAT", "-"]
]);
const REQUIRED_TEAMS = [
  { key: "IMOBILIARIAS | AGL", area: "AGL" },
  { key: "IMOBILIARIAS | CAT", area: "CAT" },
  { key: "IMOBILIARIAS | FSA", area: "FSA" },
  { key: "IMOBILIARIAS 2 | AGL", area: "AGL" },
  { key: "IMOBILIARIAS 2 | CAT", area: "CAT" },
  { key: "IMOBILIARIAS 2 | FSA", area: "FSA" },
  { key: "EQUIPE PROPRIA 2 | CAT", area: "CAT" }
];
const TEAM_MEMBER_ALLOWLIST = new Map([
  ...
  ["EQUIPE PROPRIA 2 | CAT", new Set([
    "X",
    "Y",
    "Z"
  ])],
  ["IMOBILIARIAS 2 | AGL", new Set([
    "CORRETOR IMOBILIARIAS 2 AGL"
  ])],
  ["IMOBILIARIAS 2 | FSA", new Set([
    "CORRETOR IMOBILIARIAS 2 FSA"
  ])],
  ["IMOBILIARIAS 2 | CAT", new Set([
    "CORRETOR IMOBILIARIAS 2 CAT"
  ])]
]);
const PERSON_TEAM_OVERRIDES = new Map([
  ...
  ["X", "EQUIPE PROPRIA 2 | CAT"],
  ["Y", "EQUIPE PROPRIA 2 | CAT"],
  ["Z", "EQUIPE PROPRIA 2 | CAT"],
  ["CORRETOR IMOBILIARIAS 2 AGL", "IMOBILIARIAS 2 | AGL"],
  ["CORRETOR IMOBILIARIAS 2 FSA", "IMOBILIARIAS 2 | FSA"],
  ["CORRETOR IMOBILIARIAS 2 CAT", "IMOBILIARIAS 2 | CAT"]
]);
const PERSON_DISPLAY_OVERRIDES = new Map([
  ["CORRETOR IMOBILIARIAS 2 AGL", "Corretor Imobiliárias 2 AGL"],
  ["CORRETOR IMOBILIARIAS 2 FSA", "Corretor Imobiliárias 2 FSA"],
  ["CORRETOR IMOBILIARIAS 2 CAT", "Corretor Imobiliárias 2 CAT"],
...
function canonicalTeam(value) {
  ...
  if (raw.includes("IMOBILIARIA") || raw.includes("IMOB ")) {
    const suffix = raw.match(/\b2\b/) ? " 2" : "";
    return `IMOBILIARIAS${suffix} | ${areaOf(raw)}`;
  }
...
function buildDashboard(people, managerMap, files, period, repasseTeamCounts = new Map(), realtyReservations = { totals: new Map(), details: [] }, repasseAreaCounts = new Map(), structure = { teams: new Map() }, entryDates = new Map(), yearRepasses = null, monthlyRepasses = new Map()) {
  ...
  moveTeamAfter("IMOBILIARIAS 2 | FSA", "IMOBILIARIAS | FSA");
  moveTeamAfter("IMOBILIARIAS | AGL", "EQUIPE PROPRIA 2 | AGL");
  moveTeamAfter("IMOBILIARIAS 2 | AGL", "IMOBILIARIAS | AGL");
  moveTeamAfter("IMOBILIARIAS | CAT", "EQUIPE PROPRIA | CAT");
  moveTeamAfter("EQUIPE PROPRIA 2 | CAT", "EQUIPE PROPRIA | CAT");
  moveTeamAfter("IMOBILIARIAS 2 | CAT", "IMOBILIARIAS | CAT");
